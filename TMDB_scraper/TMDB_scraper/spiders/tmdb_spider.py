...
class TmdbSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        '''
        # the url starts on the movie page
    ...
    def parse(self, response):
        '''
            Parse the movie page, get the link to the Cast & Crew page
            and then navigate to the Cast & Crew page.
        '''
        # select the attribute href that navigates to the Cast & Crew page
        next_page = response.css('p.new_button a').attrib['href']
        # get the whole link
        next_page = "https://www.themoviedb.org" + next_page
        headers = {'User-Agent': get_random_user_agent(user_agent_list)}
        # yield a request to Cast & Crew url
    ...
    def parse_actor_page(self, response):
        ...
        # get the movie's or TV show's name
        movie_or_TV_name = response.css("div.credits_list table.credit_group a.tooltip bdi::text").getall()
        # yield one dictionary for each of the movies or TV shows on which that actor has worked
